...
export default function SectionsDisplay({ sectionName,cssSections }: SectionsCodeProps) {
  ...
  const sectionTitle:string = DOMPurify.sanitize(selectedCSS ? selectedCSS.sectionName : '');
  const sectionHTML:string = DOMPurify.sanitize(selectedCSS ? selectedCSS.code.html : '');
  return(
